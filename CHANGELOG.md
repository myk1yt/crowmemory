...
### Added
- **Cleaner recall results**: Recall now applies a similarity cutoff (default 0.35) before showing hints, so you see only relevant memories instead of noise. Tune it with the `CROW_SIM_CUTOFF`, `CROW_SIM_CUTOFF_CROSS_PROJECT`, and `CROW_PROJECT_BOOST` environment variables.
- **Input sanitization**: Everything Crow ingests is scrubbed first (`crow_sanitize.py`) — stray kaomoji, emoji, and broken Korean jamo runs are cleaned while C++ code, URLs, identifiers, and normal Korean text are protected. Pure-noise input is rejected outright and never touches memory.
- **Project-aware memory**: `crow_recall` and `crow_ingest` accept an optional `project` tag. Memories from your current project surface first; other projects' memories are penalized (or hard-filtered with `strict_project=True`). Untagged memories remain global.
- **Stronger bug lessons**: Negative bug and avoidance memories now decay at full strength (`NEG_DAMPEN_BY_REGISTER`: `bug`/`life_avoid` = 1.0) instead of the previous 0.6 dampening applied to everything.
- **Maintenance scripts**: `scripts/migrate_value_bank.py` (sanitize legacy entries), `scripts/merge_value_bank.py` (merge banks), and `scripts/purge_search_debris.py` (remove search-log debris). All run in **dry-run** by default — pass `--apply` to write.
- **231-test suite**: 6 test files covering sanitization, recall precision, tool schema, and the maintenance scripts.

### Changed
- **Tool surface simplified from 10 to 3**: The MCP tools are now `crow_recall`, `crow_ingest`, and `crow_admin`. All former capabilities survive as parameters — build-result ingestion moved into `crow_ingest(exit_code=...)`, the `[User Bias]` block into `crow_recall(format="bias_block")`, and diagnostics/drift/prompt/backup/evolve/project-info into `crow_admin(action=...)`.
- **Fork files are now thin shims**: `crow_core-myk1yt.py` and `crow_mcp_server-myk1yt.py` re-export the canonical modules and delegate execution — the `-myk1yt` data set under `memory/` is a historical archive, and the live memory is `memory/crow.bin`.
- **Recall merges registers better**: Register-scoped recalls without a specific register merge results by effective similarity across the domain — no more per-register filler hints.
